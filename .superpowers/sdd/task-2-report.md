...
- A normal cancellation has three explicit outcomes. `CANCELLED` permits exit; a successful non-`CANCELLED` response may use the simulator fallback; any HTTP/network exception returns `BLOCKED` immediately and cannot issue a second cleanup request.
