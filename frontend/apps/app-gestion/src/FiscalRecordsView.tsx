  ...
  return <FiscalWorkspaceDialog id="fiscal-record-detail" variant="drawer" className="gestion-verifactu-record-detail" title={detail?.number ?? t("verifactu.records.detailTitle")} closeLabel={t("verifactu.records.closeDetail")} onClose={onClose}>
    ...
    {!loading && !error && detail && <div className="gestion-verifactu-record-detail-body">
      ...
      <div className="gestion-verifactu-record-detail-columns">
        ...
        <DetailBlock title={t("verifactu.records.submissionTitle")}><Detail label={t("verifactu.records.status")} value={detail.submission ? statusLabel(detail.submission.status, t) : t("verifactu.records.notSubmitted")} /><Detail label={t("verifactu.records.updatedAt")} value={detail.submission?.updatedAt ? formatVerifactuDateTime(detail.submission.updatedAt, locale, detailTimezone) : t("verifactu.records.none")} /><Detail label={t("verifactu.records.errorCode")} value={detail.submission?.errorCode ?? t("verifactu.records.none")} /></DetailBlock>
      </div>
    </div>}
  </FiscalWorkspaceDialog>;
}
