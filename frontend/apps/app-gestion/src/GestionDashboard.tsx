...
import {
  ...
  type DashboardWidgetLayout, type SalesOverviewData, type SalesOverviewScope, type SalesTodayData, type TopProductData
} from "./dashboardModel";
...
const defaultDataSource: DashboardDataSource = {
  ...
  loadTopProducts, loadActivePromotions, loadControlAlertsSummary, loadWarehouses: loadDashboardWarehouses
};
...
function DashboardWorkspace({ session, locale = "es", t, onOpenSales, onOpenStock, onOpenPromotions, onOpenControlAlerts, dataSource = defaultDataSource }: GestionDashboardProps) {
  ...
  const updateOptions = (patch: Partial<DashboardOptions>) => {
    if (!draft || saving) return;
    setDraft({ ...draft, options: { ...draft.options, ...patch } }); setSaveState("idle");
  };
  const openCustomization = () => {
    if (!preference) return;
    setDraft({ widgets: preference.widgets.map((widget) => ({ ...widget })), options: { ...preference.options } });
    setSelectedKey(preference.widgets.some((widget) => widget.key === "sales.trend") ? "sales.trend" : preference.widgets[0]?.key ?? "sales.today");
    setSaveState("idle");
  };
  const save = async () => {
    if (!draft || saving || !preference || preferenceError) return;
    setSaveState("saving");
    try {
  ...
  return <section className={`gestion-workspace gd-workspace ${customizing ? "is-customizing" : ""} ${options.density === "COMPACT" ? "is-compact" : ""}`}>
    <header className="gd-toolbar"><div><h2>{t(customizing ? "gestion.dashboard.customizeTitle" : "gestion.dashboard")}</h2>
      ...
      <div className="gd-toolbar-actions">
        ...
        {!customizing && <button type="button" className="gd-primary" disabled={!preference || preferenceLoading || preferenceError} onClick={openCustomization}><SlidersHorizontal size={17} aria-hidden="true" />{t("gestion.dashboard.customize")}</button>}
      </div>
    </header>
        ...
        <section className="gd-grid" aria-label={t("gestion.dashboard")}>
          {widgets.map((widget) => <DashboardWidgetFrame key={widget.key} widget={widget} customizing={customizing} selected={selectedKey === widget.key} disabled={saving} t={t}
            onSelect={() => setSelectedKey(widget.key)} onDragStart={() => setDraggedKey(widget.key)} onDrop={() => {
              ...
              setDraggedKey(null);
            }} onRemove={() => updateDraft(widgets.filter((value) => value.key !== widget.key))}>
            ...
            {widget.key === "sales.top-products" && <TopProductsWidget state={sales} t={t} locale={locale} onOpen={onOpenStock} display={options.productDisplay} />}
            {widget.key === "promotions.active" && <ActivePromotionsWidget state={promotions} t={t} locale={locale} onOpen={onOpenPromotions} date={range.to} />}
            {widget.key === "control.alerts" && <ControlAlertsWidget state={alerts} t={t} locale={locale} onOpen={onOpenControlAlerts} timeZone={preference.storeTimezone} />}
        ...
        <fieldset disabled={saving}>
          ...
          <div className="gd-config-section gd-selected-settings"><h4>{t("gestion.dashboard.selectedWidget")}</h4>
            {selected ? <><strong className="gd-selected-title">{t(`gestion.widget.${selected.key}`)}</strong>
              {(selected.key === "sales.trend" || selected.key === "sales.top-products") && <><label className="gd-config-label">{t("gestion.dashboard.visualization")}</label><div className="gd-segmented">
                {(selected.key === "sales.trend" ? ["LINE", "BAR", "TABLE"] as const : ["BAR", "TABLE"] as const).map((display) => <button type="button" key={display}
                  aria-pressed={(selected.key === "sales.trend" ? options.trendDisplay : options.productDisplay) === display}
                  onClick={() => updateOptions(selected.key === "sales.trend" ? { trendDisplay: display } : { productDisplay: display as "BAR" | "TABLE" })}>
                  {display === "LINE" ? <ChartLine size={17} aria-hidden="true" /> : display === "BAR" ? <ChartBar size={17} aria-hidden="true" /> : <Table size={17} aria-hidden="true" />}{t(`gestion.dashboard.display.${display}`)}
                </button>)}
              </div></>}
    ...
    {session.permissions.includes("ADMIN") && <div className="gestion-operational-status"><OperationalStatusCard locale={locale} token={session.accessToken} /></div>}
  </section>;
}
...
function useDashboardData<T>(loader: (signal: AbortSignal) => Promise<T>, dependencies: unknown[], enabled: boolean, scope: string): DashboardDataState<T> {
  ...
  useEffect(() => {
    ...
    setState((previous) => ({ loading: true, error: false, data: sameScope ? previous.data : undefined }));
    void loader(controller.signal).then((data) => { if (active) setState({ loading: false, error: false, data }); })
