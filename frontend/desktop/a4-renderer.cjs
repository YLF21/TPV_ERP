...
function renderA4DocumentHtml(document) {
  const normalizedTitle = String(document.title || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
  const deliveryNote = document.documentType === "ALBARAN_VENTA" || normalizedTitle.startsWith("ALBARAN");
  const labels = {
  ...
  const taxRateLabel = retail
    ? ""
    : document.fiscalProfile === "IVA" ? "IVA %" : "IGIC %";
  ...
  const payments = (deliveryNote ? [] : (document.payments || [])).map((payment) =>
  ...
  const accounts = (deliveryNote ? [] : (document.bankAccounts || [])).map((account) =>
  ...
  const standaloneLogo = !document.issuer && document.logo
    ...
    : "";
  const fiscalSnapshot = document.fiscal && typeof document.fiscal === "object" ? document.fiscal : null;
  const qrUrl = String(fiscalSnapshot?.qrUrl || document.qrUrl || "");
  const hasQr = !deliveryNote && document.qrImage && qrUrl;
  const qrCaption = [
    fiscalSnapshot?.prefix || "QR tributario:",
    fiscalSnapshot?.legend || "",
    fiscalSnapshot?.testNotice || ""
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    .standalone-logo{display:block;width:100px;max-height:70px;object-fit:contain;object-position:left top;margin-bottom:8px}.top{display:grid;grid-template-columns:minmax(0,1fr) 210px;gap:22px;min-height:150px}.top.delivery-note{min-height:92px}.issuer{display:flex;align-items:flex-start;gap:12px;padding-top:4px}.company-logo{width:100px;max-height:70px;object-fit:contain;object-position:left top}.party-data>strong{display:block;font-size:13px;margin-bottom:4px}.right-head{display:flex;flex-direction:column;align-items:stretch}.fiscal-qr{margin:0 0 8px auto;width:112px;text-align:center}.fiscal-qr img{width:92px;height:92px;display:block;margin:0 auto}.fiscal-qr figcaption{font-size:7px;line-height:1.15;margin-top:2px}.customer{border-top:1px solid #000;padding-top:7px;margin-top:auto}.delivery-note .customer{border-top:0;padding-top:4px;margin-top:0}
    ...
    .summary{display:grid;grid-template-columns:minmax(0,1fr) 240px;gap:22px;margin-top:14px}.summary.delivery-note .totals{grid-column:2}.payment-block h2,.bank-block h2,.notes h2{font-size:11px;margin:0 0 6px}.payments>div,.bank-row{display:grid;grid-template-columns:1fr auto;gap:4px 10px;padding:3px 0}.payments small{grid-column:1/-1}.totals{border-top:1px solid #000}.total-row{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #000}.total-row.final{font-size:16px;font-weight:800;padding-top:8px}.bank-block,.notes{margin-top:12px;border-top:1px solid #000;padding-top:8px}.bank-row strong{font-family:"Cascadia Mono",Consolas,monospace}.notes p{white-space:pre-wrap;margin:0}.retail-legend{margin-top:8px;font-size:9px}
  </style></head><body>${standaloneLogo}
    <header class="top${deliveryNote ? " delivery-note" : ""}">${party(document.issuer, labels, "issuer")}<div class="right-head">${qr}${party(document.customer, labels, "customer")}</div></header>
    <section class="document-data"><h1>${escapeHtml(documentHeading)}</h1><div class="document-data-grid"><span>Número</span><strong>${escapeHtml(document.documentNumber || document.title || "")}</strong><span>Fecha</span><strong>${escapeHtml(document.issuedAt)}</strong></div></section>
    <table><thead><tr><th>${escapeHtml(labels.code)}</th><th>${escapeHtml(labels.barcode)}</th><th>${escapeHtml(labels.description)}</th><th class="right">${escapeHtml(labels.quantity)}</th><th class="right">${escapeHtml(labels.unitPrice)}</th><th class="right tax-rate">${escapeHtml(retail ? "" : (labels.taxRate || taxRateLabel))}</th><th class="right">${escapeHtml(labels.total)}</th></tr></thead><tbody>${rows}</tbody></table>
    <section class="summary${deliveryNote ? " delivery-note" : ""}">${paymentSummary}<div class="totals">${Number(document.discount || 0) === 0 ? "" : `<div class="total-row"><span>${escapeHtml(labels.discount)}</span><strong>-${money(Math.abs(document.discount))}</strong></div>`}<div class="total-row"><span>${escapeHtml(labels.base)}</span><strong>${money(document.subtotal)}</strong></div>${retail ? "" : `<div class="total-row tax-total"><span>${escapeHtml(labels.tax)}</span><strong>${money(document.tax)}</strong></div>`}<div class="total-row final"><span>${escapeHtml(labels.total)}</span><strong>${money(document.total)}</strong></div></div></section>
