...
function renderA4DocumentHtml(document) {
  ...
  const metadata = (document.metadata || []).filter((entry) => entry && entry.label && entry.value).map((entry) =>
    `<div class="metadata-item"><span>${escapeHtml(entry.label)}</span><strong>${escapeHtml(entry.value)}</strong></div>`
  ).join("");
  const notes = (document.notes || []).filter(Boolean).map((note) => `<li>${escapeHtml(note)}</li>`).join("");
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    .metadata{display:grid;grid-template-columns:1fr 1fr;gap:8px 18px;border:1px solid #c8d2e0;padding:10px;margin:12px 0 20px}.metadata-item{display:flex;justify-content:space-between;gap:12px}.metadata-item span{color:#52647a}
    .notes{border:1px solid #c8d2e0;padding:10px 14px;margin-top:20px}.notes h2{font-size:14px;margin:0 0 8px}.notes ul{margin:0;padding-left:18px}
  ...
  <div class="row total"><span>${escapeHtml(labels.total)}</span><strong>${money(document.total)}</strong></div></section>
  ${notes ? `<section class="notes"><h2>${escapeHtml(labels.notes || "Notes")}</h2><ul>${notes}</ul></section>` : ""}</body></html>`;
}
