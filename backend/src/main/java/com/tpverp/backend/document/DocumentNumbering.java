...
public final class DocumentNumbering {
    ...
    public static String format(CommercialDocumentType type, LocalDate date, int sequence) {
        return format(type, date, sequence, "001");
    }

    // Formatea el consecutivo incluyendo el codigo fiscal de tienda.
    public static String format(
            CommercialDocumentType type, LocalDate date, int sequence, String storeCode) {
    ...
    public static String period(CommercialDocumentType type, LocalDate date) {
        Objects.requireNonNull(type, "tipo");
        Objects.requireNonNull(date, "fecha");
        return type.periodicity() == CommercialDocumentType.Periodicidad.DIARIA
