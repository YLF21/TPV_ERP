...
public class SalePaymentSession {
 ...
 public SalePaymentAllocation addAllocation(UUID id,String key,SalePaymentAllocationKind kind,BigDecimal amount,String provider,String mode,BigDecimal delivered,BigDecimal change,String comment){if(status!=SalePaymentSessionStatus.COLLECTING)throw new IllegalStateException("payment_session_not_collecting");if(allocations.stream().anyMatch(a->a.getIdempotencyKey().equals(key)))throw new IllegalStateException("duplicate_idempotency_key");var normalized=Money.euros(amount);var reserved=allocations.stream().filter(a->a.getStatus()!=PaymentTerminalOperationStatus.DECLINED&&a.getStatus()!=PaymentTerminalOperationStatus.ERROR&&a.getStatus()!=PaymentTerminalOperationStatus.CANCELLED).map(SalePaymentAllocation::getAmount).reduce(Money.euros(BigDecimal.ZERO),BigDecimal::add);if(normalized.signum()<=0||reserved.add(normalized).compareTo(total)>0)throw new IllegalArgumentException("allocation_exceeds_remaining");var a=SalePaymentAllocation.create(this,id,key,kind,normalized,provider,mode,delivered,change,comment);allocations.add(a);updatedAt=Instant.now();return a;}
 public SalePaymentAllocation addAllocation(UUID id,String key,SalePaymentAllocationKind kind,BigDecimal amount,String provider,String mode){return addAllocation(id,key,kind,amount,provider,mode,null,null,null);}
 ...
 public void cancel(){if(status==SalePaymentSessionStatus.FINALIZED||status==SalePaymentSessionStatus.CANCELLED)return;var requiresCompensation=allocations.stream().anyMatch(SalePaymentAllocation::requiresCompensationOnCancel);if(status==SalePaymentSessionStatus.COMPENSATION_REQUIRED&&requiresCompensation)return;status=requiresCompensation?SalePaymentSessionStatus.COMPENSATION_REQUIRED:SalePaymentSessionStatus.CANCELLED;updatedAt=Instant.now();}
