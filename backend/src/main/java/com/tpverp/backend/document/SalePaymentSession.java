...
public class SalePaymentSession {
 ...
 public void cancel(){if(status==SalePaymentSessionStatus.FINALIZED||status==SalePaymentSessionStatus.CANCELLED||status==SalePaymentSessionStatus.COMPENSATION_REQUIRED)return;var uncertainIntegrated=allocations.stream().anyMatch(a->a.requiresCompensationOnCancel());var effectiveApproved=allocations.stream().anyMatch(a->a.getKind()!=SalePaymentAllocationKind.VOUCHER&&a.getStatus()==PaymentTerminalOperationStatus.APPROVED);if(effectiveApproved||uncertainIntegrated)status=SalePaymentSessionStatus.COMPENSATION_REQUIRED;else status=SalePaymentSessionStatus.CANCELLED;updatedAt=Instant.now();}
