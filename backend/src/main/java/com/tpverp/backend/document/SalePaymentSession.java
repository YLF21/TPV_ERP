...
public class SalePaymentSession {
 ...
 public void discardSimulation(String reason,UUID userId){if(reason==null||reason.isBlank())throw new IllegalArgumentException("simulator_discard_reason_required");var resolvedBy=Objects.requireNonNull(userId);if(status==SalePaymentSessionStatus.FINALIZED)throw new IllegalStateException("payment_session_finalized");compensationNote=reason.trim();compensationResolvedBy=resolvedBy;compensationResolvedAt=Instant.now();status=SalePaymentSessionStatus.CANCELLED;updatedAt=compensationResolvedAt;}
