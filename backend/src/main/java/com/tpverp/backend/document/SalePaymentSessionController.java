...
public class SalePaymentSessionController {
 ...
 public record IssuedVoucherView(String code,BigDecimal amount,java.time.Instant issuedAt,String originTicketNumber,List<VoucherPrintService.TraceView> traceability,String observations,VoucherPrintService.RenderedContent renderedPdf,VoucherPrintService.RenderedContent ticketRenderedImage){static IssuedVoucherView from(SalePaymentSessionService.IssuedVoucher voucher){return voucher==null?null:new IssuedVoucherView(voucher.code(),voucher.amount(),voucher.issuedAt(),voucher.originTicketNumber(),voucher.traceability(),voucher.observations(),voucher.renderedPdf(),voucher.ticketRenderedImage());}}
