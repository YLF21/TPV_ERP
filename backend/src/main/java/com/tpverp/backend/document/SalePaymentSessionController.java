...
public class SalePaymentSessionController {
 ...
 @PostMapping public View reserve(@Valid @RequestBody Reserve request,Authentication auth){return view(service.reserve(request.sessionId(),request.sale(),request.memberBalanceReservationId(),auth));}
 ...
 private View view(SalePaymentSession session,TicketPrintView printTicket,List<TicketPrintView> additionalPrintTickets,TicketPrintView nonFiscalSummary,SalePaymentSessionService.IssuedVoucher issuedVoucher){return View.from(session,printTicket,additionalPrintTickets,nonFiscalSummary,issuedVoucher,service.refundPaymentAvailability(session),service.requiresVoucherOnlyRefund(session));}
 public record Reserve(@NotNull UUID sessionId,@NotNull @Valid PosCashController.SaleRequest sale,UUID memberBalanceReservationId){public Reserve(UUID sessionId,PosCashController.SaleRequest sale){this(sessionId,sale,null);}}
 ...
 public record IssuedVoucherView(String code,String familyIdentifier,BigDecimal amount,java.time.Instant issuedAt,java.time.LocalDate expiresOn,String originTicketNumber,List<VoucherPrintService.TraceView> traceability,String observations,VoucherPrintService.RenderedContent renderedPdf,VoucherPrintService.RenderedContent ticketRenderedImage){static IssuedVoucherView from(SalePaymentSessionService.IssuedVoucher voucher){return voucher==null?null:new IssuedVoucherView(voucher.code(),voucher.familyIdentifier(),voucher.amount(),voucher.issuedAt(),voucher.expiresOn(),voucher.originTicketNumber(),voucher.traceability(),voucher.observations(),voucher.renderedPdf(),voucher.ticketRenderedImage());}}
  public record View(UUID id,BigDecimal total,BigDecimal documentTotal,SalePaymentSessionDirection direction,String currency,String status,UUID ticketId,String ticketNumber,UUID memberBalanceReservationId,BigDecimal memberBalanceRequestedAmount,BigDecimal memberBalanceAppliedAmount,BigDecimal memberReturnCreditRequestedAmount,BigDecimal memberReturnCreditAppliedAmount,String memberBalanceFailureCode,java.time.Instant memberBalanceSynchronizedAt,List<AllocationView> allocations,TicketPrintView printTicket,List<TicketPrintView> additionalPrintTickets,TicketPrintView nonFiscalSummary,IssuedVoucherView issuedVoucher,List<RefundPaymentAvailability.View> refundPaymentAvailability,boolean voucherOnlyRefund){static View from(SalePaymentSession s){return from(s,null,null,List.of(),false);}static View from(SalePaymentSession s,TicketPrintView printTicket){return from(s,printTicket,null,List.of(),false);}static View from(SalePaymentSession s,TicketPrintView printTicket,SalePaymentSessionService.IssuedVoucher issuedVoucher){return from(s,printTicket,issuedVoucher,List.of(),false);}static View from(SalePaymentSession s,List<RefundPaymentAvailability.View> availability,boolean voucherOnlyRefund){return from(s,null,null,availability,voucherOnlyRefund);}static View from(SalePaymentSession s,TicketPrintView printTicket,SalePaymentSessionService.IssuedVoucher issuedVoucher,List<RefundPaymentAvailability.View> availability,boolean voucherOnlyRefund){return from(s,printTicket,List.of(),null,issuedVoucher,availability,voucherOnlyRefund);}static View from(SalePaymentSession s,TicketPrintView printTicket,List<TicketPrintView> additionalPrintTickets,TicketPrintView nonFiscalSummary,SalePaymentSessionService.IssuedVoucher issuedVoucher,List<RefundPaymentAvailability.View> availability,boolean voucherOnlyRefund){return new View(s.getId(),s.getTotal(),s.getDocumentTotal(),s.getDirection(),s.getCurrency(),s.getStatus().name(),s.getTicketId(),s.getTicketNumber(),s.getMemberBalanceReservationId(),s.getMemberBalanceRequestedAmount(),s.getMemberBalanceAppliedAmount(),s.getMemberReturnCreditRequestedAmount(),s.getMemberReturnCreditAppliedAmount(),s.getMemberBalanceFailureCode(),s.getMemberBalanceSynchronizedAt(),s.getAllocations().stream().map(AllocationView::from).toList(),printTicket,additionalPrintTickets==null?List.of():List.copyOf(additionalPrintTickets),nonFiscalSummary,IssuedVoucherView.from(issuedVoucher),List.copyOf(availability),voucherOnlyRefund);}}
}
