...
public class SalePaymentSessionController {
 ...
 private View view(SalePaymentSession session){return View.from(session,service.refundPaymentAvailability(session));}
 private View view(SalePaymentSession session,TicketPrintView printTicket,SalePaymentSessionService.IssuedVoucher issuedVoucher){return View.from(session,printTicket,issuedVoucher,service.refundPaymentAvailability(session));}
 ...
 public record View(UUID id,BigDecimal total,BigDecimal documentTotal,SalePaymentSessionDirection direction,String currency,String status,UUID ticketId,String ticketNumber,List<AllocationView> allocations,TicketPrintView printTicket,IssuedVoucherView issuedVoucher,List<RefundPaymentAvailability.View> refundPaymentAvailability){static View from(SalePaymentSession s){return from(s,null,null,List.of());}static View from(SalePaymentSession s,TicketPrintView printTicket){return from(s,printTicket,null,List.of());}static View from(SalePaymentSession s,TicketPrintView printTicket,SalePaymentSessionService.IssuedVoucher issuedVoucher){return from(s,printTicket,issuedVoucher,List.of());}static View from(SalePaymentSession s,List<RefundPaymentAvailability.View> availability){return from(s,null,null,availability);}static View from(SalePaymentSession s,TicketPrintView printTicket,SalePaymentSessionService.IssuedVoucher issuedVoucher,List<RefundPaymentAvailability.View> availability){return new View(s.getId(),s.getTotal(),s.getDocumentTotal(),s.getDirection(),s.getCurrency(),s.getStatus().name(),s.getTicketId(),s.getTicketNumber(),s.getAllocations().stream().map(AllocationView::from).toList(),printTicket,IssuedVoucherView.from(issuedVoucher),List.copyOf(availability));}}
}
