...
@Import({
        ...
        InventoryDocumentGatewayConcurrencyPostgreSqlTest.Configuration.class
})
...
class InventoryDocumentGatewayConcurrencyPostgreSqlTest {
    ...
    @Autowired private InventoryDocumentGateway gateway;
    ...
    void concurrentTicketConfirmationsPreserveStockAndMovementLedger() throws Exception {
        ...
        for (int round = 0; round < 20; round++) {
            ...
            confirmTogether(context.storeId(), fixture.firstDocumentId(), fixture.secondDocumentId());
    ...
    private void confirmTogether(UUID storeId, UUID firstDocumentId, UUID secondDocumentId)
            throws Exception {
        var ready = new CountDownLatch(2);
        var start = new CountDownLatch(1);
        try (var pool = Executors.newFixedThreadPool(2)) {
            var first = pool.submit(confirm(firstDocumentId, storeId, ready, start));
            var second = pool.submit(confirm(secondDocumentId, storeId, ready, start));
