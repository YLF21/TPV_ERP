...
class FiscalRuntimeGuardInitializerTest {
    ...
    @Test
    void v238MarkerIsUpgradedToTheImmutableV240ReleaseIdentity() {
        var jdbc = jdbc(marker("SANDBOX", "DUAL", "tpv-erp-dev-v238", "V238", null, null,
                null, 7, 0));
        when(jdbc.queryForList(anyString(), eq(String.class)))
                .thenReturn(List.of("238", "239", "240"));

        new FiscalRuntimeGuardInitializer(jdbc,
                sandbox("tpv-erp-dev-v240", "DEV", FiscalProductCapability.DUAL, 8, 0, "V240"))
                .run(new DefaultApplicationArguments());

        verifyMarkerUpdate(jdbc, "tpv-erp-dev-v240", 8L, 0L);
    }
    ...
    @Test
    void packagedV263ManifestCanAdvanceAnExistingV262Database() {
        ...
        var jdbc = jdbc(marker("SANDBOX", manifest.capability().name(),
                development ? "tpv-erp-dev-v262" : "tpv-erp-4.2.0-v262",
                "V262", null, null, null, development ? 22 : 14, development ? 0 : 1));
        when(jdbc.queryForList(anyString(), eq(String.class)))
                .thenReturn(List.of("261", "262", "263"));
        ...
        when(runtime.productCapability()).thenReturn(manifest.capability());

        new FiscalRuntimeGuardInitializer(jdbc, runtime).run(new DefaultApplicationArguments());
        ...
        verify(jdbc).update(org.mockito.ArgumentMatchers.startsWith("update fiscal_runtime_guard"),
                ...
                eq("V263"), eq(manifest.manifestHash()), org.mockito.ArgumentMatchers.isNull(),
