...
class MigrationV228OperationalQueueContractTest {

    @Test
    void definesPartialConcurrentIndexesForScopedActiveQueueReads() throws Exception {
        var sql = Files.readString(Path.of(
                "src/main/resources/db/migration/V228__verifactu_operational_queue_read_indexes.sql"));
        var config = Files.readString(Path.of(
                "src/main/resources/db/migration/V228__verifactu_operational_queue_read_indexes.sql.conf"));
